...
# bit array to be represented with the checkboxes
  def bitToCheckbox(self, boxArray, bitArray):
  ...
  def mar_pc_Load(self, arr, buttons):
      bits = ""
      for i in range(4, 16):
        if str(buttons[i].cget("bg")) == "blue":
          self.colorChange(buttons, i)
          bits += '1'
        else:
          bits += '0'
      print(bits)
      self.bitToCheckbox(arr, bits)
      
  def mbrLoad(self, buttons, mbr):
    bits = ""
    for i in range(0, 16):
      if str(buttons[i].cget("bg")) == "blue":
        self.colorChange(buttons, i)
        ...
        bits += '0'
    print(bits)
    self.bitToCheckbox(mbr, bits)
    
  def colorChange(self, buttons, button):
  ...
  def run(self):
    ...
    pc_LD= tk.Button(otherFrame, text = "LD", fg = "green",padx=8,pady=5,relief=tk.RAISED, bg="#97ecf7", command=lambda: self.mar_pc_Load(pc, buttons)).grid(row=1,column=35)
    ...
    mar_LD = tk.Button(otherFrame, text = "LD", fg = "green",padx=8,pady=5,relief=tk.RAISED, bg="#97ecf7", command=lambda: self.mar_pc_Load(mar, buttons)).grid(row=2,column=35)
    ...
    mbr_LD = tk.Button(otherFrame, text = "LD", fg = "green",padx=8,pady=5,relief=tk.RAISED, bg="#97ecf7", command=lambda: self.mbrLoad(buttons, mbr)).grid(row=3,column=35)
    ...
    init_btn = tk.Button(buttonFrame, text = "Init", fg = "red",padx=8,pady=8,relief=tk.RAISED, bg="#97ecf7").grid(row=20,column=3)
    ...
    for i in range (0,16):
      btn_no += 1
      buttons.append(tk.Button(InstructionFrame, text=(-1)*(btn_no-15), bg="#97ecf7", command= lambda x=btn_no: self.colorChange(buttons, x)))
